...
func SocketClient() {
	...
	for size:=1; size<=maxsize; size*=2 {
		...
		th := 2*8*float64(size)*float64(iter)/elasped_time/1024/1024
...
func recv(rbuf []byte, sbuf []byte, conn net.Conn) (int) {
	recvbyte := 0	
	for {
		//recvbyte = 0	
...
func send(sbuf []byte, rbuf []byte, conn net.Conn) {
	_, err := conn.Write(sbuf)
	if err != nil {
		if err != nil {
			log.Fatal(err)
